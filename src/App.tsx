...
import {
  ...
  ReceiptText, Box, CircleDollarSign, Receipt, ChevronDown,
} from "lucide-react";
...
function AppContent() {
  ...
  return (
   <div className={`flex h-screen transition-colors duration-300 font-sans antialiased ${settings.theme === "dark" ? "bg-gray-900 text-white" : "bg-gradient-to-br from-gray-50 via-blue-50/30 to-gray-50"} overflow-hidden`}>
      ...
      <aside className={`${isSidebarOpen ? "w-72" : "w-20"} bg-[#D1F4FA] dark:bg-gray-900 border-r border-gray-200/50 dark:border-gray-800 shadow-xl transition-all duration-300 flex flex-col`}>
              ...
              <>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <h1 className={`text-xl font-extrabold ${settings.theme === "dark" ? "text-white" : "text-gray-900"}`}>HotelPro</h1>
                  </div>
                </div>
                <button onClick={() => setIsSidebarOpen(false)} className="p-2 hover:bg-black/5 rounded-lg transition-colors"><X className="w-5 h-5" /></button>
              </>
            ) : (
              <button onClick={() => setIsSidebarOpen(true)} className="p-2 hover:bg-black/5 rounded-lg mx-auto"><Menu className="w-5 h-5" /></button>
            )}
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          <div className="space-y-1">
            ...
            {/* 1. Dashboard Tab */}
            {mainMenuTabs.filter(t => t.id === "dashboard").map((tab) => {
              const Icon = tab.icon;
              return (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${activeTab === tab.id ? `bg-gradient-to-r ${themeColors[settings.primaryColor] || themeColors.blue} text-white shadow-lg` : `${settings.theme === "dark" ? "text-gray-300 hover:bg-gray-700" : "text-gray-800 hover:bg-gray-100"}`}`}>
                  <Icon className="w-5 h-5 flex-shrink-0" />
                  {/* Chuyển font-medium thành font-semibold */}
                  {isSidebarOpen && <span className="font-semibold text-[15px]">{tab.label}</span>}
                </button>
              );
            })}
...
<div className="pt-2">
  <button
    ...
    className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 ${
      isReceptionOpen && isSidebarOpen ? "bg-blue-600/10 text-blue-700 font-bold" : "text-gray-800 hover:bg-gray-100"
    }`}
  >
    <div className="flex items-center gap-3">
      <LogIn className="w-5 h-5 flex-shrink-0" />
      {/* Sử dụng t() để dịch tiêu đề Quầy Lễ Tân */}
      {isSidebarOpen && <span className="font-bold uppercase text-sm tracking-wider">{t('sidebar.reception')}</span>}
    </div>
    {isSidebarOpen && (
       <ChevronDown size={14} className={`transition-transform duration-300 ${isReceptionOpen ? "" : "-rotate-90"}`} />
    )}
  </button>
  ...
  {isReceptionOpen && isSidebarOpen && (
    <div className="mt-1 ml-4 border-l-2 border-blue-200 pl-2 space-y-1 animate-in slide-in-from-top-1">
     ...
     <button onClick={() => setActiveTab("checkinout")} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-[15px] font-semibold transition-all ${activeTab === "checkinout" ? "bg-blue-50 text-blue-700" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"}`}>
        <Bed size={16} /> <span>{t('sidebar.room')}</span> 
      </button>
      
      <button onClick={() => setActiveTab("guests")} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-[15px] font-semibold transition-all ${activeTab === "guests" ? "bg-blue-50 text-blue-700" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"}`}>
        <Users size={16} /> <span>{t('sidebar.customer')}</span>
      </button>
      
      <button onClick={() => setActiveTab("bookings")} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-[15px] font-semibold transition-all ${activeTab === "bookings" ? "bg-blue-50 text-blue-700" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"}`}>
        ...
        <div className="p-4 border-t border-gray-200/50">
          ...
          {isSidebarOpen && (
            <div className={`p-4 rounded-xl border transition-colors ${settings.theme === "dark" ? "bg-gray-800 border-gray-700" : "bg-white/80 border-blue-200 backdrop-blur-sm shadow-sm"}`}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[15px] font-bold text-gray-900 dark:text-white truncate">{currentUser?.name}</p>
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400 truncate">{currentUser?.email}</p>
                </div>
              </div>
              <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 text-red-600 rounded-lg hover:bg-red-50 hover:border-red-300 transition-all text-[15px] font-bold border border-red-200 shadow-sm">
